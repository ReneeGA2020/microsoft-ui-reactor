# DuctD3 Chart Authoring Guide

DuctD3 is a C# port of D3.js that renders charts as Duct Element trees on WinUI3 Canvas. Charts are pure functions: `data => Element`. The goal is a single `return D3Canvas(...)` expression per chart with no imperative mutation.
...
using static Duct.D3.Charts.D3;   // D3Rect, D3Circle, D3LinePath, Brush, Gray, etc.
using static Duct.UI;              // Text, Canvas, ScrollView, etc.
```
...
All return Duct Elements. Customize with `with { }` for Fill, Stroke, etc.
