...
- [ ] `src/vscode-reactor/src/extension.ts`: change the spawn args from `--preview --vscode` to `--devtools run --vscode`.
- [ ] Keep one-release compat: if the target Reactor package is older, fall back to `--preview --vscode`. Detect by first-line stdout sniff (`[devtools] ready` vs `[preview] …`).
- [ ] Update extension's user-facing labels ("Reactor Preview" panel title is unchanged; internal telemetry event names get a `_devtools` variant with the old name kept as an alias for one release).
