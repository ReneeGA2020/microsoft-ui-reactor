# Duct Source Mapping — Design Spec
...
When inspecting a live Duct app (via `--preview` mode or a future element
...
PDB files at runtime. Mark Duct framework methods with
...
```csharp
// Duct/Diagnostics/SourceInfo.cs

#if DEBUG
using Microsoft.UI.Xaml;

namespace Duct.Diagnostics;
...
- Or use the Lazy PDB approach (Approach 6): capture IL offsets cheaply,
  ...
  it
- `[DebuggerNonUserCode]` on all Duct framework code enables "Just My Code"
  style filtering

### Visual Studio integration

The `SourceInfo.LocationProperty` attached property is already visible in VS's
XAML Live Visual Tree. A VS extension could add a "Go to Duct Source" context
