# Duct Navigation — Design Spec

Declarative, type-safe navigation for Duct: a developer-owned navigation stack
...
Today, Duct developers manually switch components via `UseState`:
...
3. **Parameterless constructors** — `ActivationAPI::ActivateInstance()` calls
   the constructor with zero arguments. Duct components take props.
...
around them requires either XAML adapter files (breaking Duct's pure-C# model)
...
- Automatic navigation from XAML NavigationView.MenuItems — Duct controls
...
**Takeaway:** Duct's navigation state should be a typed list managed via a hook.
...
│  └──────┬───────┘  └──────┬───────┘  └───────┬───────┘ │
│         │                 │                   │         │
│         │    NavigationContext (DuctContext)   │         │
│         └─────────────────┼───────────────────┘         │
│                           │                             │
├───────────────────────────┼─────────────────────────────┤
│  Existing Duct layers     │                             │
│                           ▼                             │
│  ┌────────────────────────────────────────────────────┐ │
...
stack, and distributes a `NavigationHandle<TRoute>` via `DuctContext` so
...
class AppShell : Component
{
    public override Element Render()
    {
        ...
        // Also published to DuctContext so children can UseNavigation<AppRoute>()
        ...
    }
}
```
...
3. Publishes via `DuctContext<NavigationHandle<TRoute>>` to the subtree.
...
/// Provided via DuctContext to all descendants of the component that
...
accelerated animation system used by Duct's existing `LayoutAnimation` and
...
struct CachedPage
{
    ...
    Element LastElement;           // The Duct element tree (for reconciliation)
...
- `NavigationContext<TRoute>` — `DuctContext` for sharing the handle

**New files:**
- `Duct/Core/Navigation/NavigationStack.cs`
- `Duct/Core/Navigation/NavigationHandle.cs`
- `Duct/Core/Navigation/NavigationHostElement.cs` (or inline in Element.cs)
- `Duct/Core/Navigation/NavigationTransition.cs`
- `Duct/Core/Navigation/NavigationLifecycle.cs`

**Modified files:**
- `Duct/Core/RenderContext.cs` — add `UseNavigation<T>()` and
  `UseNavigationLifecycle()` hooks
- `Duct/Core/Component.cs` — add convenience methods delegating to
  `RenderContext`
- `Duct/Core/Element.cs` — add `NavigationHostElement` record
- `Duct/Elements/Dsl.cs` — add `NavigationHost()` factory
- `Duct/Core/Reconciler.Mount.cs` — add `MountNavigationHost()`
- `Duct/Core/Reconciler.Update.cs` — add `UpdateNavigationHost()`
...
**New files:**
- `Duct/Core/Navigation/TransitionEngine.cs`

**Modified files:**
- `Duct/Core/Reconciler.Mount.cs` — transition logic in NavigationHost handler
...
**New files:**
- `Duct/Core/Navigation/NavigationCache.cs`
- `Duct/Core/Navigation/DeepLinkMap.cs`
...
When we can make changes to the WinUI3 runtime, the goal is to make Duct
...
This means Duct can pass `TypeName { Name = "MyApp.Routes.Detail", Kind = ... }`
...
This allows Duct pages to participate in Frame's built-in LRU caching.
...
### From manual state switching (current Duct pattern)
...
### From DuctPageHelper + XAML Frame navigation
...
`DuctPageHelper` remains available for hybrid apps that mix XAML pages and Duct
...
Create a single XAML-backed `DuctBridgePage : Page` and use WinUI's Frame
normally.

**Rejected because:**
- Requires at least one XAML file, breaking the pure-C# model.
- Creates a new `DuctHost` per navigation (performance overhead).
...
- Two state management systems (Duct hooks + Frame back stack) must stay in sync.
...
- Cannot ship Duct navigation until WinUI changes land.

**Preserved as future work:** Once Duct's native navigation proves the design,
the WinUI changes described in "Future: WinUI3 Runtime Integration" can be
proposed upstream. This gives Duct navigation today AND deeper integration later.

### Alternative 3: Type-safe router layer over WinUI Frame

Build the Duct router API (type-safe routes, hooks) but internally delegate to
a WinUI Frame + bridge page for rendering and transitions.

**Rejected because:**
- Dual state management (Duct stack + Frame stack) is a bug surface.
- Still requires one XAML file.
- Creates new DuctHost per page.
- Nested navigation is constrained by Frame.
- Gains are marginal vs. the pure Duct approach — WinUI's slide transition is
...
|----------|-------------------|--------|-----------|
| Navigation state ownership | Framework-owned (Frame), Developer-owned (hook) | Developer-owned | Industry trend (SwiftUI, Compose Nav3); fits Duct's hook model; testable |
...
| Transition system | WinUI Frame transitions, CSS-like, Composition APIs | Composition APIs | Already used by Duct (LayoutAnimation); GPU-accelerated; full control |
...
| Context distribution | Props threading, DuctContext, global static | DuctContext | Consistent with Duct's existing context system; no prop drilling |
