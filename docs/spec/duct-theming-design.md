# Duct Theming & Styling System — Detailed Design
...
│  → Uses Duct's reactive re-render (like React Context)      │
...
public static class Theme
{
    ...
    /// The resource must be defined via DuctApp.DefineThemeResources()
...
### DuctThemeResources — Custom App-Level Theme Definitions
...
/// </summary>
public class DuctThemeResources
{
...
ThemeScope(
    resources: new DuctThemeResources()
...
DuctApp.Run<MyApp>("My App", new DuctAppOptions
{
    ThemeResources = new DuctThemeResources
    {
...
{
    public DuctThemeResources? Resources { get; init; }
    public ElementTheme? RequestedTheme { get; init; }
    ...
    private void InjectResources(FrameworkElement fe)
    {
        ...
        foreach (var (key, value) in Resources.Resources)
        {
            switch (value)
            {
                case DuctThemeResources.ThemedValue.BrushValue bv:
                    lightDict[key] = bv.Light;
                    darkDict[key] = bv.Dark;
                    break;
                case DuctThemeResources.ThemedValue.DoubleValue dv:
...
### Phase 6: DuctApp.DefineThemeResources()
...
public static class DuctApp
{
    ...
    /// </summary>
    public static void DefineThemeResources(DuctThemeResources resources)
    {
...
DuctApp.DefineThemeResources(new DuctThemeResources
{
...
public static class LimeTheme
{
    ...
    public static DuctThemeResources Create()
    {
        var resources = new DuctThemeResources();
...
DuctApp.DefineThemeResources(LimeTheme.Create());

DuctApp.Run<MainPage>("Lime App");
...
1. `DuctApp.DefineThemeResources()` injects brushes into
...
3. Standard controls get lime green without Duct setting ANY local values. The properties
...
4. When the OS switches Light ↔ Dark, WinUI re-resolves all ThemeResource references
   and picks up the appropriate variant from our ThemeDictionaries. No Duct re-render needed
   for the built-in controls.

5. Custom resources (`"BrandSubtle"`, etc.) referenced via `Theme.Ref(...)` use Tier 2
   (ThemeRef), so Duct re-renders those components on theme change to re-resolve the value.

---

## Scenario 2: Custom Duct Component with Theme-Aware Properties

This scenario shows how to author a reusable Duct component — a `StatusCard` — that
...
2. **WinUI built-in controls are free** — Button, Slider, TextBox, ToggleSwitch etc.
   ...
   No Duct re-render needed; WinUI handles this natively at the framework level.

3. **Duct Theme tokens also respect the scope** — When `Theme.PrimaryText` is used
...
6. **Performance** — `RequestedTheme` is the most efficient theme mechanism because
   ...
   zero Duct overhead. Only Duct components that use `Theme.*` tokens need a re-render
...
5. Default style
6. Inheritance
7. Default value        ← Tier 3: property not set by Duct
...
The difference is behavioral:
  ...
  - Tier 2: Set at render time, RE-SET when theme changes (Duct re-renders)

Tier 3 properties are never touched by Duct, so WinUI's style system
has full control (ThemeResource bindings, visual states, etc.).
```

**Why Duct re-render (not WinUI live binding) for Tier 2:**

| Approach | Pros | Cons |
|----------|------|------|
| **Duct re-render** (chosen) | Simple, works for any property, custom tokens, subtree themes | Re-render on theme change (rare event, acceptable) |
...
| `DuctApp.DefineThemeResources()` | N resources × 2 themes | Once at startup |
...
### The Problem: What Happens When Duct Sets a Local Value

**Scenario:** Duct sets `button.Background = redBrush` (a local value), then the
user hovers over the button.

**Timeline:**

```
Step 1: Duct reconciler sets  button.Background = redBrush
...
```
Button.Background = redBrush                    ← LOCAL VALUE (Duct sets this)
...
**Result: Visual states DO work for standard controls even with Duct's local value!**
...
TemplateBinding on an inner element), then Duct's local value and the VSM animation
target the SAME element. In this case:

```
Duct sets: control.SomeProperty = value        ← LOCAL on control
VSM sets:  control.SomeProperty via animation  ← ANIMATED on same element
```

Due to the Silverlight-era flag logic:
- If animation starts AFTER Duct set the value → `SetAnimatedValue()` clears the
  "newer" flag → animation wins during active state ✓
- If Duct re-renders WHILE animation is active → sets new local value → sets
...
unless Duct sets ContentPresenter.Background as a local value (which it can't,
...
### Design Decision: Three Strategies for Duct

Given this analysis, Duct should support three strategies, chosen by the developer:
...
- **Visual states:** Partially work — PointerOver/Pressed animations on the
  ...
  But the "rest" state is locked to your color. If Duct re-renders during an active
...
### Also Discovered: Duct Doesn't ClearValue on Null Transitions

A secondary issue found during this analysis: when a Duct element's modifier goes
...
- [ ] DuctThemeResources class
- [ ] DuctApp.DefineThemeResources() — inject into Application.Resources
...
2. **Theme change animation?**
   Should Duct support animated transitions between themes
...
4. **XAML style interop?**
   Should Duct be able to load `.xaml` resource dictionaries for theme
