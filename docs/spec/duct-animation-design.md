# Duct Animation — Design Spec

Eight compositor-layer animation features that close the gaps identified in the
[critical review](../duct-critical-review.md) section 10, lifting Duct's animation
...
The [critical review](../duct-critical-review.md) §10 grades Duct's animation at
...
animatable. Compose's `animateAsState` does the same. Duct cannot match this
...
| API | What it enables | Current Duct usage |
|-----|----------------|--------------------|
...
**Key file**: New `Duct\Animation\Curve.cs`
...
| File | Change |
|------|--------|
| `Duct\Animation\AnimationScope.cs` | New — `[ThreadStatic]` scope + `WithAnimation` |
| `Duct\Animation\AnimationHelper.cs` | New — `SetOrAnimate` / compositor animation creation |
| `Duct\Core\Reconciler.cs` | Modify — `ApplyModifiers()` routes through `AnimationHelper` |
...
| File | Change |
|------|--------|
| `Duct\Animation\Transition.cs` | New — transition type hierarchy |
| `Duct\Core\Element.cs` | Modify — add `ElementTransition?` property to Element base |
| `Duct\Core\Reconciler.cs` | Modify — animate on mount, delay unmount on exit |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.Transition()` fluent modifier |
| `Duct\Core\Navigation\TransitionEngine.cs` | Reuse — compositor animation patterns |
...
| File | Change |
|------|--------|
| `Duct\Animation\AnimateProperty.cs` | New — flags enum |
| `Duct\Core\Element.cs` | Modify — add `AnimationConfig?` to Element base |
| `Duct\Core\Reconciler.cs` | Modify — `ApplyPropertyAnimation()` parallel to `ApplyLayoutAnimation()` |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.Animate()` fluent modifier |
...
| File | Change |
|------|--------|
| `Duct\Core\Element.cs` | Modify — add `StaggerConfig?` to container element types |
| `Duct\Core\Reconciler.cs` | Modify — apply `DelayTime` in animation setup paths |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.Stagger()` fluent modifier |
...
| File | Change |
|------|--------|
| `Duct\Animation\KeyframeBuilder.cs` | New — builder API + data records |
| `Duct\Core\Element.cs` | Modify — keyframe animation storage on Element |
| `Duct\Core\Reconciler.cs` | Modify — trigger keyframe animations on mount/state change |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.Keyframes()` fluent modifier |
...
| File | Change |
|------|--------|
| `Duct\Animation\ScrollAnimation.cs` | New — builder + expression templates |
| `Duct\Core\Element.cs` | Modify — add scroll-linked animation storage |
| `Duct\Core\Reconciler.cs` | Modify — apply expression animations on mount/update |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.ScrollLinked()` fluent modifier |
...
| File | Change |
|------|--------|
| `Duct\Animation\AnimationScope.cs` | Modify — add `WithAnimationAsync()` |
| `Duct\Core\Navigation\TransitionEngine.cs` | Reference — existing batch pattern |
...
Today, hover/pressed visual feedback in Duct requires state variables and a
...
| File | Change |
|------|--------|
| `Duct\Animation\InteractionStates.cs` | New — config records, animation factory, state machine handler |
| `Duct\Core\Element.cs` | Modify — add `InteractionStatesConfig?` property |
| `Duct\Core\Reconciler.cs` | Modify — register handlers, create/cache animations on mount, respect interaction state during modifier application |
| `Duct\Elements\ElementExtensions.cs` | Modify — `.InteractionStates()` fluent modifier |
...
| File | Purpose |
|------|---------|
| `Duct\Animation\Curve.cs` | Curve/Easing type hierarchy |
| `Duct\Animation\AnimationScope.cs` | `WithAnimation` / `WithAnimationAsync` |
| `Duct\Animation\AnimationHelper.cs` | `SetOrAnimate` — routes property sets through compositor |
| `Duct\Animation\Transition.cs` | Enter/exit transition type hierarchy |
| `Duct\Animation\AnimateProperty.cs` | Flags enum for `.Animate()` property targeting |
| `Duct\Animation\KeyframeBuilder.cs` | Keyframe builder API + data records |
| `Duct\Animation\ScrollAnimation.cs` | Scroll-linked expression builder + templates |
| `Duct\Animation\InteractionStates.cs` | Interaction state config records + animation factory |

## Modified File Summary

| File | Changes |
|------|---------|
| `Duct\Core\Element.cs` | Add `ElementTransition?`, `AnimationConfig?`, `InteractionStatesConfig?`, stagger, keyframe, scroll-linked properties |
| `Duct\Core\Reconciler.cs` | `ApplyModifiers` → animate via scope; `ApplyPropertyAnimation`; enter/exit lifecycle; stagger delays; keyframe triggers; scroll expression setup; interaction state handler registration |
| `Duct\Elements\ElementExtensions.cs` | `.Transition()`, `.Animate()`, `.InteractionStates()`, `.Stagger()`, `.Keyframes()`, `.ScrollLinked()` fluent modifiers |
...
- **Unit tests**: Verify compositor animations are created with correct parameters.
  Follow the pattern in `tests\Duct.AppTests.Host\SelfTest\Fixtures\LayoutAnimationFixtures.cs`
...
- **Demo app**: Extend `docs\apps\animation\App.cs` with interactive sections for
  each feature.
- **Test app**: Add sections to `samples\Duct.TestApp\App.cs` transitions page
  (~line 1370+).
...
| Capability | Duct (current) | Duct (proposed) | SwiftUI | Compose | Flux.UI |
|-----------|---------------|----------------|---------|---------|---------|
