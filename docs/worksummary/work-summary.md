...
| **Sample Apps** | 8 | Mar 20 – Apr 14 | DuctFiles file explorer, Monaco Editor, DuctOutlook (Outlook clone), NetPulse stress test, HeadTrax employee DB, StressPerf benchmarks, TestApp enhancements, WinUI vanilla host demo |
...
| **Testing Infrastructure & Coverage** | 6 | Mar 21 – Apr 13 | In-process self-test harness, Appium/WinAppDriver E2E (182s to 41s), selfhost fixture system, render-idle detection, coverage expansion (Core 75%, Controls 88%, Hooks 68%, D3 80%), 312 DuctD3 tests, flaky test fixes |
...
| **Perf Experiments & DuctCpp** | 3 | Mar 30 – Apr 4 | DuctCpp fully native C++ reconciler (built, measured, archived), NativeAOT, property diff bitmask, interactive control pooling, arena allocation experiments, benchmark suite |
...
| **DuctD3 Charting Library** | 2 | Apr 1 – Apr 12 | Full charting library, 30+ gallery samples, declarative D3 DSL, SVG icons, functional generators (LinePath, AreaPath, ArcPath, Pie), chart DSL migration to native D3 element trees |
...
| **Commanding System** | 1 | Apr 8 | DuctCommand, StandardCommand, UseCommand hook, CommandHost, perf benchmark, MenuBar fix |
...
Each feature is rated against the following scale based on implementation completeness, test coverage, bug-fix iterations, and known gaps documented in the [critical review](../duct-critical-review.md). The **Review Grade** column reflects the scorecard from the competitive analysis (vs React, SwiftUI, Compose).
...
| **DuctD3 Charting Library** | Production | B+ | Full charting library with 30+ test classes (80% line coverage). Declarative D3 DSL, functional generators, native element trees. Complete and independently usable. |
...
| **Localization System** | Beta | B+ | Full Phases 1–6: ICU runtime, source generator, pseudolocalization, CLI (extract/translate/validate/status/prune), RichMessage API, RTL. Migrated to DuctContext. Gap: standard command labels not yet localized. |
...
| **Sample Apps** | Prototype | — | 10 sample apps (DuctOutlook, HeadTrax, DuctFiles, Monaco, NetPulse, StressPerf, etc.). Serve as integration tests and demos. Gap: older apps don't adopt newest features (navigation, context, commanding). |
...
| **Perf Experiments & DuctCpp** | Archived | — | DuctCpp C++ reconciler: built, benchmarked, concluded overhead didn't justify complexity. Rust differ: same arc (week 1 add, week 2 remove). Property diff bitmask, control pooling, arena allocation all evaluated. Findings documented in spec 007. |
...
|---|---|---|
| Production | 7 | Core Framework, Yoga Flex, DuctD3, Commanding, Markdown, Testing, Documentation |
...
| Archived / Meta | 2 | Perf Experiments/DuctCpp, Code Review Remediation |
...
- **Rapid experimentation:** DuctCpp was a deliberate 3-day spike — built a full C++ reconciler, benchmarked it, concluded the perf gains didn't justify the complexity, and archived findings. Same pattern with the Rust differ (added week 1, removed week 2).
