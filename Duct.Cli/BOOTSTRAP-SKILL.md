...
name: duct-cli
description: >
  Bootstrap skill for the Duct CLI tool (patch.exe). Patch is a React-inspired functional UI
  framework for WinUI 3 — declarative C# with no XAML. Use this when asked to "create a Duct app",
  "build a desktop app with Duct", "scaffold a WinUI project", or anything involving the Duct
  framework. This skill tells you how to call the CLI; the CLI itself contains the full API reference.
---

# Duct CLI — Bootstrap Skill

## What is Patch?

Duct (Functional UI) is a **React-inspired declarative C# framework for WinUI 3**. It replaces
...
| `--create <name>`  | Scaffold a new Duct project (`.sln`, `.csproj`, `Program.cs`)         |
...
- **Entry point** is a single top-level statement: `DuctApp.Run<MyComponent>("Title");`
