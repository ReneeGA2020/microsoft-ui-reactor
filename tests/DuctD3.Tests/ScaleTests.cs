// Tests for new scale types: Ordinal, Band, Point, Log, Pow, Quantize, Quantile, Threshold
...
public class OrdinalScaleTests
{
    ...
    public void Map_ImplicitDomain()
    {
        ...
        Assert.Equal(10, s.Map("a")); // implicitly added
        Assert.Equal(20, s.Map("b"));
        Assert.Equal(10, s.Map("a")); // same as before
    }
    ...
    public void Map_WrapsRange()
    {
        ...
        Assert.Equal(10, s.Map("c")); // wraps around
...
public class LogScaleTests
{
    ...
    public void Map_LogTransform()
    {
        ...
        // log10(10) = 1, log10(100) = 2 → (1 - 0)/(2 - 0) = 0.5
