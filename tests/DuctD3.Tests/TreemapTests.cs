// Tests for treemap and pack layouts
...
public class TreemapTests
{
    ...
    public void Treemap_LeafRectanglesCoverArea()
    {
        ...
        // All leaves should have non-zero area
...
public class PackTests
{
    ...
    public void Pack_RootEncloses()
    {
        ...
        // Children should be within root circle (approximately)
        foreach (var child in node.Children)
        {
            double dist = Math.Sqrt(
                (child.X - node.X) * (child.X - node.X) +
                (child.Y - node.Y) * (child.Y - node.Y));
            Assert.True(dist + child.R <= node.R + 1); // small tolerance
        }
    }
}
