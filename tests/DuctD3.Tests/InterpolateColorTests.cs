// Tests for color/date/array interpolation
...
public class InterpolateColorTests
{
    ...
    public void Hsl_InterpolatesHue()
    {
        var interp = D3InterpolateColor.Hsl(
            new D3Color(255, 0, 0),   // red, hue=0
            new D3Color(0, 0, 255));  // blue, hue=240

        var mid = interp(0.5);
        // Midpoint hue between 0° and 240° via shortest path (through 120°) should be green-ish
        // But shortest path from 0° to 240° goes backward through 300°/360° (cyan-magenta), not through green
        // So the midpoint is at hue=120° or hue=300° depending on direction
        // Just verify it interpolates (not same as start or end)
        Assert.False(mid.R == 255 && mid.G == 0 && mid.B == 0); // not start
        Assert.False(mid.R == 0 && mid.G == 0 && mid.B == 255); // not end
    }
