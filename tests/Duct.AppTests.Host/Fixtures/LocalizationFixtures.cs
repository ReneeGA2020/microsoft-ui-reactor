    ...
    internal class LocaleSwitchingComponent : Component
    {
        ...
        public override Element Render()
        {
            ...
            return VStack(8,
                ...
                // Main content panel
                VStack(8,
                    Text(t.Message(AppTitle)).Set(tb => tb.FontSize = 24).AutomationId("LocTitle"),
                    Text(t.Message(Welcome)).AutomationId("LocWelcome"),
                    Text(t.Message(Greeting, new { name = "World" })).AutomationId("LocGreeting"),

                    // Plural scenarios
                    Text(t.Message(ItemCount, new { count = 0 })).AutomationId("LocPluralZero"),
                    Text(t.Message(ItemCount, new { count = 5 })).AutomationId("LocPluralFive"),

                    // Search-results with multiple plural + param
                    Text(t.Message(SearchResults, new { count = 0, query = locale == "ko-KR" ? "\ud14c\uc2a4\ud2b8" : "test" }))
                        .AutomationId("LocSearchZero"),
                    Text(t.Message(SearchResults, new { count = 1, query = locale == "ko-KR" ? "\ud14c\uc2a4\ud2b8" : "test" }))
                        .AutomationId("LocSearchOne"),
                    Text(t.Message(SearchResults, new { count = 42, query = locale == "ko-KR" ? "\ud14c\uc2a4\ud2b8" : "test" }))
                        .AutomationId("LocSearchMany"),

                    // Direction label
                    Text(t.Message(DirectionLabel)).AutomationId("LocDirection")
                )
