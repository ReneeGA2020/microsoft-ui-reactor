...
///   These require WinUI Application context and run in Duct.AppTests or self-test fixtures.
/// </summary>
public class NavigationHostTests
{
    ...
    {
        Home => Text("Home Page"),
        Detail d => Text($"Detail #{d.Id}"),
        Settings => Text("Settings Page"),
        Profile p => Text($"Profile: {p.Name}"),
        _ => Text("Unknown"),
    };
    ...
    public void Child_Component_Can_Navigate_Parents_Stack()
    {
        ...
        scope.Push(new Dictionary<DuctContextBase, object?> { [NavigationContext<Route>.Instance] = nav });
