...
/// are converted to the correct Duct Element tree structure.
/// Tests the public API via UI.Markdown(...) which delegates to MarkdownBuilder.Build().
///
...
public class MarkdownBuilderTests
{
    ...
    /// <summary>Build markdown to Element tree (avoids namespace/method name clash with Duct.Markdown).</summary>
    private static Element Md(string markdown) => UI.Markdown(markdown);

    /// <summary>Build markdown to Element tree with custom options.</summary>
    private static Element Md(string markdown, MarkdownOptions options) => UI.Markdown(markdown, options);
    ...
    public void CodeBlock_Callback_ElementAppearsInTree()
    {
        var options = new MarkdownOptions
        {
            CodeBlock = (code, lang) => Text($"CODE:{code}")
        };
    ...
    public void HtmlBlock_Callback_ElementAppearsInTree()
    {
        var options = new MarkdownOptions
        {
            HtmlBlock = (html) => Text("CUSTOM_HTML")
        };
