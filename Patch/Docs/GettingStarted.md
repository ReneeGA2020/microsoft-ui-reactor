...
A complete Patch application in a single file:
...
PatchApp.Run<HelloWorld>("My App");
...
```csharp
using Patch;                   // PatchApp, UI (DSL), element extensions
using Patch.Core;              // Component, Element, RenderContext
...
```csharp
// Simple
PatchApp.Run<MyComponent>("Window Title");

// With size
PatchApp.Run<MyComponent>("Window Title", width: 800, height: 600);

// Inline root
PatchApp.Run("My App", ctx =>
{
