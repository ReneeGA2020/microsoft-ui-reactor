...
name: patch-cli
description: >
  Bootstrap skill for the Patch CLI tool (patch.exe). Patch is a React-inspired functional UI
  framework for WinUI 3 — declarative C# with no XAML. Use this when asked to "create a Patch app",
  "build a desktop app with Patch", "scaffold a WinUI project", or anything involving the Patch
  framework. This skill tells you how to call the CLI; the CLI itself contains the full API reference.
---

# Patch CLI — Bootstrap Skill

## What is Patch?

Patch (Functional UI) is a **React-inspired declarative C# framework for WinUI 3**. It replaces
...
| `--create <name>`  | Scaffold a new Patch project (`.sln`, `.csproj`, `Program.cs`)         |
...
- **Entry point** is a single top-level statement: `PatchApp.Run<MyComponent>("Title");`
