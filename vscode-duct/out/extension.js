...
function activate(context) {
    ...
    outputChannel = vscode.window.createOutputChannel("Duct Preview");
    statusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Right, 100);
    statusBarItem.command = "duct.previewFocus";
    context.subscriptions.push(statusBarItem);
    context.subscriptions.push(vscode.commands.registerCommand("duct.preview", () => startAutoPreview(context)), vscode.commands.registerCommand("duct.previewConnect", () => connectToPreview(context)), vscode.commands.registerCommand("duct.previewStop", stopPreview), vscode.commands.registerCommand("duct.previewFocus", focusPreviewWindow));
}
function deactivate() {
    stopPreview();
...
async function startAutoPreview(context) {
    ...
    if (!editor || editor.document.languageId !== "csharp") {
        vscode.window.showWarningMessage("Open a C# file containing a Duct Component, then run this command.");
        return;
    }
    ...
    if (!editorChangeDisposable) {
        editorChangeDisposable = vscode.window.onDidChangeActiveTextEditor(async (newEditor) => {
            ...
            if (newCsproj !== currentCsprojPath) {
                ...
                outputChannel.appendLine(`[duct] Project changed to ${path.basename(newCsproj)}, relaunching...`);
                await launchPreviewProcess(extensionContext, newCsproj);
                return;
            }
            // Same project — switch component via HTTP (instant)
            outputChannel.appendLine(`[duct] Editor switched to ${path.basename(newFilePath)}, switching to ${fileComponents[0]}...`);
            await switchComponentViaHttp(fileComponents[0]);
        });
    }
}
...
async function launchPreviewProcess(context, csprojPath) {
    if (isLaunching) {
        outputChannel.appendLine(`[duct] Already launching, ignoring request`);
        return;
    }
    isLaunching = true;
    ...
    outputChannel.appendLine(`[duct] Launching: dotnet ${args.join(" ")}`);
    ...
    previewProcess.stdout?.on("data", (data) => {
        ...
        if (match) {
            ...
            outputChannel.appendLine(`[duct] Capture server on port ${capturePort}`);
    ...
    previewProcess.on("exit", (code) => {
        outputChannel.appendLine(`[duct] Preview process exited with code ${code}`);
        statusBarItem.text = "$(circle-slash) Duct Preview: Stopped";
        setTimeout(() => {
            if (!previewProcess)
                statusBarItem.hide();
        }, 5000);
...
async function switchComponentViaHttp(componentName) {
    ...
    try {
        ...
        if (result.ok) {
            ...
            statusBarItem.text = `$(eye) Duct: ${componentName}`;
            statusBarItem.tooltip = `Previewing ${componentName} — port ${capturePort}\nClick to focus window`;
            if (panel) {
                panel.title = `Duct: ${componentName}`;
            ...
            outputChannel.appendLine(`[duct] Switched to ${componentName} via HTTP`);
        }
        else {
            outputChannel.appendLine(`[duct] Switch failed: ${result.error}`);
        }
    }
    catch (err) {
        outputChannel.appendLine(`[duct] Failed to switch component: ${err}`);
    }
}
...
async function connectToPreview(context) {
    ...
    capturePort = port;
    statusBarItem.text = "$(eye) Duct Preview";
...
function showPreviewPanel(context, componentName) {
    ...
    panel = vscode.window.createWebviewPanel("ductPreview", `Duct: ${componentName}`, { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
        enableScripts: true,
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        if (msg.type === "selectComponent" && msg.name) {
            outputChannel.appendLine(`[duct] Component selected from dropdown: ${msg.name}`);
...
function updatePanelHtml() {
    if (!panel || !capturePort)
        return;
    panel.webview.html = getWebviewHtml(capturePort, currentComponents, currentComponentName ?? currentComponents[0]);
}
...
<body>
  ...
  <div class="preview-container">
    <img id="preview" alt="Duct Preview" style="display:none" />
