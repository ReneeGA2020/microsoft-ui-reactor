...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("duct.preview", () =>
      startAutoPreview(context)
    ),
    vscode.commands.registerCommand("duct.previewConnect", () =>
      connectToPreview(context)
    ),
    vscode.commands.registerCommand("duct.previewStop", stopPreview),
    vscode.commands.registerCommand("duct.previewFocus", focusPreviewWindow)
  );
}
...
async function startAutoPreview(context: vscode.ExtensionContext) {
  ...
  if (!editor || editor.document.languageId !== "csharp") {
    vscode.window.showWarningMessage(
      "Open a C# file containing a Duct Component, then run this command."
    );
    return;
  }
      ...
      async (newEditor) => {
        ...
        if (newCsproj !== currentCsprojPath) {
          ...
          outputChannel.appendLine(
            `[duct] Project changed to ${path.basename(newCsproj)}, relaunching...`
          );
        ...
        outputChannel.appendLine(
          `[duct] Editor switched to ${path.basename(newFilePath)}, switching to ${fileComponents[0]}...`
        );
        await switchComponentViaHttp(fileComponents[0]);
      }
    );
...
async function switchComponentViaHttp(componentName: string) {
  ...
  try {
    ...
    if (result.ok) {
      ...
      outputChannel.appendLine(
        `[duct] Switched to ${componentName} via HTTP`
      );
    } else {
      outputChannel.appendLine(
        `[duct] Switch failed: ${result.error}`
      );
    }
  } catch (err) {
    outputChannel.appendLine(
      `[duct] Failed to switch component: ${err}`
    );
  }
}
...
function showPreviewPanel(
  ...
  panel.webview.onDidReceiveMessage(async (msg) => {
    if (msg.type === "selectComponent" && msg.name) {
      outputChannel.appendLine(
        `[duct] Component selected from dropdown: ${msg.name}`
      );
