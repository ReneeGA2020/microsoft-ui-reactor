# DuctCpp: Native C++ Reconciler and Language Binding

This document specifies a fully native C++ implementation of the Duct framework — element model, hooks, reconciler, and WinUI bridge — designed so that an existing C++ program can adopt WinUI declaratively without a ground-up rewrite.

---

## Motivation

The C# Duct framework pays a managed/unmanaged transition cost on every WinRT call during reconciliation. The Rust native differ experiment (now removed) showed that a native component embedded in a mostly-managed stack doesn't net enough perf win — the serialization boundary ate the gains.
...
5. **Perf comparison** — add a 4th `StressPerf.DuctCpp` variant to the existing benchmark harness.
6. **Isolated experiment** — everything lives under a single top-level `DuctCpp/` directory; easy to delete if the experiment doesn't pan out.
...
We surveyed the existing C++ UI framework landscape to inform DuctCpp's design. **No existing C++ framework combines React-style hooks, virtual element tree reconciliation, and a bridge to a native platform UI toolkit.** DuctCpp is novel in this regard. The closest conceptual matches and their relevant patterns:
...
4. **Reconciliation is the missing piece** — none of the surveyed C++ frameworks implement virtual tree diffing. This is DuctCpp's core differentiator, enabling efficient declarative UI with minimal platform control churn.
...
```
DuctCpp/                               # Top-level isolated directory
├── DuctCpp.sln                        # VS solution (all projects)
│
├── DuctCpp/                           # Core static library (.vcxproj)
│   ├── DuctCpp.vcxproj
│   ├── packages.config
...
│       ├── element_pool.cpp
│       ├── host.h                    # DuctHost (render loop)
│       ├── host.cpp
│       └── winrt_bridge.cpp          # WinRT property setters, control creation
│
├── DuctCpp.TestApp/                   # WinUI 3 test app (.vcxproj)
│   ├── DuctCpp.TestApp.vcxproj
│   ├── packages.config
│   ├── main.cpp                       # Entry point
│   └── app.cpp                        # Port of Duct.TestApp (all 11 demos)
│
└── DuctCpp.StressPerf/               # Benchmark variant (.vcxproj)
    ├── DuctCpp.StressPerf.vcxproj
    ├── packages.config
    └── main.cpp                       # StressPerf.DuctCpp variant
```
...
Direct port of `DuctHost.cs`:

```cpp
class DuctHost {
public:
    DuctHost(winrt::Microsoft::UI::Xaml::Window window);
...
| Scenario | Direct | Bound | Duct (C#) | DuctCpp |
|----------|--------|-------|-----------|---------|
...
The hypothesis: DuctCpp should approach Direct-like performance because the diff is native and property sets are the only WinRT calls (same as Direct). The C# Duct variant pays for managed element allocation + GC + WinRT interop throughout reconciliation.
...
- [ ] Set up `DuctCpp/` directory and MSBuild solution (.sln + .vcxproj)
...
- [ ] Implement `DuctHost` render loop with `DispatcherQueue`
...
- [ ] Implement `StressPerf.DuctCpp` with same grid layout
...
**Goal:** Make the call on whether DuctCpp is worth continuing.
