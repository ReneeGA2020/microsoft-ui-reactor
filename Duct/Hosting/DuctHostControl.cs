...
/// <summary>
/// A fully self-contained WinUI ContentControl that hosts a Duct component tree.
/// Drop this into any vanilla WinUI app — no DuctApp, DuctApplication, or special
/// bootstrapping needed. Each instance owns its own Reconciler and render loop.
///
/// Usage in XAML:
///   <![CDATA[
///   <local:DuctHostControl x:Name="ductHost" />
///   ]]>
///
/// Usage in code-behind:
///   ductHost.Mount(new MyComponent());
///   — or —
///   ductHost.Mount(ctx => VStack(Text("Hello from Duct!")));
///   — or via XAML property —
///   <![CDATA[
///   <local:DuctHostControl ComponentFactory="{x:Bind CreateMyComponent}" />
///   ]]>
///
/// Features:
