...
/// <summary>
/// Builds a Duct Element tree from markdown using the md4c SAX parser.
/// </summary>
internal sealed class MarkdownBuilder
{
    ...
    /// <summary>
    /// Parse markdown and build a Duct Element tree.
    /// </summary>
    ...
    private void LeaveListItem(object? detail)
    {
        ...
        if (frame.Children.Count == 1)
            ...
            content = Text(""); // empty fallback

        Element element = HStack(4,
            Text(marker).VAlign(VerticalAlignment.Top),
            content
        );
    ...
    private void LeaveHtmlBlock()
    {
        ...
        if (_options.HtmlBlock is not null)
            ...
            element = Text(html).Foreground("#888888");
