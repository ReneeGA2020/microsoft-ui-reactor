...
A complete Reactor application in a single file:
...
ReactorApp.Run<HelloWorld>("My App");
...
```csharp
using Microsoft.UI.Reactor;                   // ReactorApp, UI (DSL), element extensions
using Microsoft.UI.Reactor.Core;              // Component, Element, RenderContext
...
> the Reactor `Flex()` / `FlexRow()` / `FlexColumn()` containers and `.Flex()` child modifier
> work the same way. See `Reactor/Docs/specs/flex-layout.md` for the full spec.
...
Reactor has first-class support for WinUI transitions. Transition configuration is stored declaratively on each element and applied by the reconciler — no `.Set()` or `.OnMount()` needed.
...
```csharp
// Simple
ReactorApp.Run<MyComponent>("Window Title");

// With size
ReactorApp.Run<MyComponent>("Window Title", width: 800, height: 600);

// Inline root
ReactorApp.Run("My App", ctx =>
{
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <ProjectReference Include="..\Patch\Reactor.csproj" />
  </ItemGroup>
</Project>
```
