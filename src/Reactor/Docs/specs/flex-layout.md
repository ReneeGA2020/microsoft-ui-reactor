...
This document specifies a CSS Flexbox layout panel for WinUI3, powered by a pure C# port of Meta's [Yoga](https://github.com/facebook/yoga) layout engine. The implementation is split into three layers: a standalone Yoga port, a standalone WinUI3 FlexPanel, and Reactor integration.

---

## Goals

1. **Pure C# Yoga port** — isolated, no WinUI or Reactor dependencies, updatable independently.
2. **Standalone FlexPanel** — usable in any WinUI3 app via XAML markup with no Reactor dependency.
3. **Reactor integration** — first-class `Flex()` element and attached property extensions on par with `Grid()`, `VStack()`, etc.

## Non-Goals

- CSS Grid support (Yoga has experimental Grid, but it's not stable enough to ship).
- Virtualization (see analysis below).
- Replacing existing Reactor layout primitives — Flex is additive.
...
The ergonomic advantage is significant. Every WinUI3 developer understands panels. The XAML is cleaner, the attached property syntax is more natural (`FlexPanel.Grow` vs `Flex.Grow` on an unrelated `LayoutPanel`), and we avoid requiring developers to know about the `LayoutPanel` control. The "standalone, no Reactor dependency" goal also benefits — a single `FlexPanel` class is much easier to extract and distribute than a `FlexLayout` + instructions to use `LayoutPanel`.
...
│  (depends on Reactor.Core + Reactor.Flex)                  │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│              Reactor.Flex (Standalone)                   │
│  FlexPanel : Panel                                   │
│  FlexPanel attached DPs (Grow, Shrink, Basis, etc.)  │
│  (depends on WinUI3 + Reactor.Layout only)              │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│              Reactor.Layout (Standalone)                  │
...
## Layer 1: Reactor.Layout — Pure C# Yoga Port

### Namespace: `Reactor.Layout`
...
## Layer 2: Reactor.Flex — Standalone FlexPanel

### Namespace: `Reactor.Flex`
...
## Layer 3: Reactor Integration

### Namespace: `Reactor` (elements) + `Reactor.Core` (records)
...
samples/FlexPanelGallery/            # Standalone XAML test app (no Reactor dependency)
...
A WinUI3 app that exercises FlexPanel entirely through XAML markup, with no Reactor dependency. Modeled after the WinUI Gallery pattern: a NavigationView shell with one page per feature area.

### Purpose

1. **Validate the standalone story** — prove FlexPanel works in a pure XAML app with zero Reactor code.
...
<Project Sdk="Microsoft.NET.Sdk">
  ...
  <ItemGroup>
    ...
    <!-- Project reference to Reactor only for the Yoga + Flex namespaces. No Reactor.Core usage. -->
    <ProjectReference Include="..\Reactor\Reactor.csproj" />
  </ItemGroup>
</Project>
```

The project references Reactor's assembly to get `FlexPanel` and the Yoga types, but **imports only `Reactor.Yoga` and `Reactor.Flex` namespaces** — no `using Microsoft.UI.Reactor;`, no `using Microsoft.UI.Reactor.Core;`. This enforces the isolation contract at the code level.
...
2. **FlexPanel** — build the Panel subclass, test with XAML-only scenarios (no Reactor).
3. **FlexPanelGallery** — build the standalone XAML test app, validate all properties visually.
4. **Reactor integration** — add the element/attached/DSL, wire up mount/update.
...
1. **Assembly split**: Should `Reactor.Layout` and `Reactor.Flex` be separate assemblies/NuGet packages, or just separate namespaces within the Reactor project? Separate assemblies maximize extractability but add build complexity.
