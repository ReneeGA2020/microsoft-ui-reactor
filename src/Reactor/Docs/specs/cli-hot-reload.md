# CLI Hot Reload for Reactor Apps
...
Hot reload works in Visual Studio because VS computes Roslyn deltas and pushes them into the running process. But there's no way to launch a Reactor app from the CLI and get hot reload when editing in VS Code (or any other editor).
...
1. Builds and launches a Reactor app
2. Watches `.cs` files for changes
3. Computes Roslyn deltas and pushes them to the running process
4. Triggers `MetadataUpdateHandler` → `ReactorApp.ActiveHost.RequestRender()` (already implemented)
...
```
  ┌─────────────┐         named pipe          ┌──────────────────────┐
  │  duct dev    │ ──── delta bytes ────────►  │  ReactorHotReloadAgent  │
...
### Component 1: `Reactor.DevHost` (CLI-side delta generator)

A new project (or addition to `Reactor.Cli`) that:
...
4. Watches `.cs` files with `FileSystemWatcher` (debounced, same pattern as ReactorFiles sample)
5. On change:
...
### Component 2: `Reactor.HotReloadAgent` (in-process startup hook)
...
The existing `HotReloadService.cs` already handles the `UpdateApplication` callback — no changes needed to the Reactor framework itself.
...
```
Reactor.HotReloadAgent/
  Reactor.HotReloadAgent.csproj    # net8.0, no WinUI dependency
...
The `duct dev` command can be added to `Reactor.Cli` or as a separate `Reactor.DevHost` project. Adding to `Reactor.Cli` is simpler since the CLI already exists, but the Roslyn/MSBuild dependencies are heavy. A separate project keeps the CLI lean.

```
Reactor.DevHost/
  Reactor.DevHost.csproj           # net8.0, references Roslyn + MSBuild workspace
...
1. Create `Reactor.HotReloadAgent` project
...
1. Create `Reactor.DevHost` project
2. Implement build + process launch with correct env vars
3. Implement file watcher (adapt ReactorFiles pattern)
...
1. Add `duct dev` command to `Reactor.Cli` that delegates to `Reactor.DevHost`
...
2. **Separate project vs. extend Reactor.Cli?** The Roslyn/MSBuild dependencies are ~50MB of NuGet packages. Adding them to `Reactor.Cli` would bloat the tool. A separate `Reactor.DevHost` exe keeps things clean but means two binaries.
