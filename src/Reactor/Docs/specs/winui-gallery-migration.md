...
This document tracks the features needed to incrementally migrate the WinUI Gallery app from XAML to Reactor. The Gallery is a 122-page XAML app with a NavigationView shell, Frame-based page navigation, and a repeating `ControlExample` pattern on every page. The goal is to migrate pages one at a time without rewriting the shell or navigation infrastructure upfront.
...
## Feature 2: ReactorHost Control (Embed Reactor inside XAML)
...
Reactor currently owns the entire window content tree via `ReactorApp`/`ReactorHost`. There is no way to embed a Reactor component tree inside a XAML page or control. This means migration is all-or-nothing — you must rewrite the entire app at once.

### Design

Create a `ReactorHost` class that is a WinUI `ContentControl` (or `UserControl`) which can be placed in XAML and renders a Reactor component tree inside itself.

```xml
<!-- In any XAML page -->
<local:ReactorHost Component="typeof(MyReactorComponent)" />
```

#### Requirements

- `ReactorHost` is a `FrameworkElement` that XAML can instantiate.
- It accepts a component type (and optionally props) and renders the Reactor tree as its content.
- It owns its own `Reconciler` instance and render loop.
- It stretches to fill its parent by default.
- It propagates the WinUI `ActualTheme` to the Reactor tree so theme resources resolve correctly.
- It handles cleanup (unmount, hook cleanups) when the host is unloaded from the visual tree.
- Multiple `ReactorHost` instances can coexist on the same page or across pages.

#### API sketch

```csharp
public class ReactorHost : ContentControl
{
...
- Should `ReactorHost` accept a `Component` instance or a `Type`? A `Type` is simpler for XAML (no instantiation in markup). A `Func<Element>` or `FuncElement` could also work for lightweight usage.
- Should it share a `Reconciler` with other `ReactorHost` instances, or each get their own? Separate is safer (no cross-contamination of component state).

---

## Feature 3: Reactor Page Adapter (Frame Navigation)
...
The Gallery navigates between pages using `Frame.Navigate(typeof(SomePage))`. Every control page is a WinUI `Page` subclass. To migrate a single page to Reactor, we need a `Page` that hosts a Reactor component — essentially `ReactorHost` wrapped in a `Page` with navigation parameter support.

### Design

```csharp
public class ReactorPage<TComponent> : Page where TComponent : Component, new()
{
    private ReactorHost _host;

    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        _host = new ReactorHost { ComponentType = typeof(TComponent) };
    ...
    {
        // Unmount Reactor tree, run cleanups
        _host.Dispose();
    }
}
```
...
// After (Reactor page):
{ "ButtonPage", typeof(ReactorPage<ButtonPageComponent>) }
```
...
- The Reactor component receives the parameter as a prop (needs a convention — e.g., `INavigationAware` interface or a `Props` property on `Component`).
...
The Gallery uses WinUI theme resources extensively: `TextFillColorSecondaryBrush`, `ControlFillColorDefaultBrush`, `OverlayCornerRadius`, text styles like `BodyTextBlockStyle`, etc. These resolve differently in light vs. dark mode. Reactor has no way to reference named theme resources — you'd have to hardcode colors, which breaks theme switching.
...
Nearly every Gallery page uses `VisualStateManager` with `AdaptiveTrigger MinWindowWidth="641"` to switch between wide (side-by-side) and narrow (stacked) layouts. Reactor components render the same regardless of window size — there's no equivalent of adaptive triggers.
...
- Needs access to the `Window` instance. Could be provided via a context/ambient value set by `ReactorHost` or `ReactorApp`.
...
The Gallery's data layer uses `INotifyPropertyChanged` (`SettingsHelper`, `ItemsPageBase`) and `ObservableCollection<T>` (`ControlInfoDataItem` collections). Migrated Reactor pages need to consume these data sources without rewriting them. Reactor's hook-based state model has no bridge to the .NET observable pattern.
...
Could be provided as extension methods on `RenderContext` or as static helpers in a `Reactor.Interop` namespace.

---

## Feature 7: Reverse Embedding (XAML inside Reactor)
...
If the shell (NavigationView, TitleBar, search) is eventually migrated to Reactor, any remaining XAML pages need to be embeddable as children in the Reactor tree. This is the inverse of Feature 2.
...
The Gallery uses `DrillInNavigationTransitionInfo`, `RepositionThemeTransition`, implicit animations (Community Toolkit), and connected animations. Reactor has no built-in animation support.

### Design

#### Navigation transitions

Handled by the `Frame` itself (Feature 3), not by Reactor. The `ReactorPage<T>` participates in Frame transitions because it's a real `Page`. No Reactor work needed.
...
The Gallery passes navigation parameters to pages (e.g., `UniqueId` strings). Feature 3 (`ReactorPage<T>`) needs a way to pass these parameters into the Reactor component. Currently, `ComponentElement` has an `object? Props` field, but `Component` has no typed way to receive props.
...
#### Integration with ReactorPage

```csharp
public class ReactorPage<TComponent, TProps> : Page
...
| 2. ReactorHost Control | Embed Reactor component tree inside XAML | Complete |
| 3. ReactorPage Adapter | `Page` subclass for Frame navigation | Complete |
...
| Feature | Description | Status |
|---------|-------------|--------|
| 7. Reverse Embedding | XAML pages inside Reactor tree (pattern on top of Feature 1) | Complete |
...
- [x] **Build Reactor ControlExample component**: Wrap the existing XAML `ControlExample` custom control using Feature 1, so migrated pages can reuse it.
...
- [x] **Migrate shell pages**: Application-level pages migrated to Reactor components.
...
- [x] **Migrate shell**: NavigationView chrome, TitleBar, search box.
    - [x] ShellComponent renders NavigationView + Frame via Reactor DSL
...
## Reactor Framework Gaps (discovered during migration)

These gaps were found during the migration of 105 control pages, 6 shell pages, and the NavigationView shell, representing areas where the Reactor DSL forced workarounds via `.Set()`, `.OnMount()`, or full imperative construction. Fixed gaps have been removed. Ordered by impact.
...
| **No way to embed existing XAML UserControls as Reactor elements** | HomePage, ItemPage | HomePageHeader, PageHeader embedded via Border + `.Set()` workaround; requires RegisterType for clean integration |
